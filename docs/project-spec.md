...
Flutter V1 仅支持 Android 和 iOS，采用在线优先模式，服务端与 MySQL 是业务
事实来源。客户端使用 feature-first 目录和 `data / application /
presentation` 三层边界：

- Riverpod 负责状态管理与依赖装配；
- Dio 负责 HTTP 通信，认证接口与业务接口使用独立实例；
- GoRouter 使用保留标签栈的三标签导航；
- 金额在业务模型中使用 Decimal，不能使用 `double` 计算；
- Token 只保存到系统安全存储，Refresh Token 轮换必须单飞执行；
- dev 和 prod 使用独立应用身份与编译配置，生产环境只允许 HTTPS；
- V1 不提供离线写入，不在本地建立第二套持仓事实数据。

已确认的首页视觉基线见
[Fund Keeper 首页设计稿](design/fund-keeper-home-v1.png)。
...
V1 为每只基金维护一个 `primarySector`，用于首页“持仓主题分布”：

- 每只基金只能有一个主主题，无法可靠判断时归入 `OTHER`；
- 系统可以根据基金资料提供建议，但不得把名称猜测冒充权威分类；
- 用户可以修正主主题；
- 首页按各主题当前市值占全部当前市值的比例生成环形图；
- 该图表示基金主题分类，不表示基金底层股票的实时行业暴露。
...
场内 ETF 也遵循金额优先录入：只有购买金额时，可使用可追溯的成交时段市场
价格推算后台份额，但记录保持为 `ESTIMATED`。用户补充券商最终成交份额或
可验证成交明细后才能确认。当前实时行情只负责当前市值计算，不得覆盖或
证明历史成交价格。
...
- `OFF_EXCHANGE`：使用基金公司已公布的正式单位净值；
- `EXCHANGE_TRADED`：使用交易所二级市场价格；
- 混合持仓：逐只计算后汇总，接口返回 `priceType=MIXED`。

更新策略：

- 交易时段后端每 60 秒更新活跃场内 ETF 行情；
- 0～90 秒视为正常；
- 90 秒～3 分钟标记更新延迟；
- 超过 3 分钟视为过期，降级展示最近正式净值；
- 场外基金不生成盘中估值，当日正式净值未公布时今日收益为空。

第一版不自研估值算法，场内行情通过统一适配接口接入：
...
学习版使用东方财富公开市场行情，只适合个人学习和自用。实际商用供应商、
授权方式、调用额度和费用仍需单独确认。
...
Redis 不是交易流水和持仓的唯一存储。Redis 故障时，正式持仓查询仍应可用，只失去场内实时行情和缓存加速。
...
- [ ] 用户可以创建多个平台账户。
- [ ] 用户可以按基金代码、金额和时间录入交易。
- [x] 支持 `POSITION_SNAPSHOT` JSON 导入。
- [x] 支持完整 `TRANSACTION_BATCH` JSON 导入（批量买入、部分卖出和全部卖出）。
- [x] 批量买入支持预检、确认、幂等、原子回滚和快照时间边界。
- [x] 支持手动部分卖出、全部卖出、移出成本和已实现收益。
- [x] 新建估算买入支持用平台份额确认或按前置快照撤销。
- [x] 持仓快照导入支持预检、确认、幂等、回滚和错误报告。
- [x] 快照校准预检展示当前值、目标值和持仓差异。
