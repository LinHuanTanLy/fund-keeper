...
class PortfolioSellIntegrationTests {
    ...
    @Test
    void fundDetailCombinesAccountBreakdownOpenActivityAndHistory()
            throws Exception {
        Session session = preparedPosition();
        sell(
                session.token(),
                sellBody(
                        "sell-detail-confirmed-001",
                        session.accountId(),
                        "PARTIAL",
                        "420.00",
                        "390.00",
        ...
        mockMvc.perform(post("/api/v1/transactions/buys")
                        ...
                        .content("""
                                {
                                  "requestId": "buy-detail-second-001",
                                  "accountId": "%s",
                                  "fundCode": "000001",
                                  "amount": 200.00,
                                  "submittedDate": "2026-07-24",
        ...
        mockMvc.perform(get("/api/v1/portfolio/funds/000001")
                ...
                .andExpect(jsonPath(
                        ...
                        is(400.0)))
                .andExpect(jsonPath(
                        "$.data.summary.holdingCost",
                        is(800.0)))
                .andExpect(jsonPath(
                        "$.data.summary.realizedProfit",
                        is(-10.0)))
                .andExpect(jsonPath(
                        "$.data.summary.openSellCount",
                        is(1)))
                .andExpect(jsonPath(
                        "$.data.accounts",
                        hasSize(2)))
                .andExpect(jsonPath(
                        ...
                        is(session.accountId())))
                .andExpect(jsonPath(
                        "$.data.accounts[0].shares",
                        is(300.0)))
                .andExpect(jsonPath(
                        "$.data.accounts[0].realizedProfit",
                        is(-10.0)))
                .andExpect(jsonPath(
                        "$.data.accounts[0].openSellCount",
                        is(0)))
                .andExpect(jsonPath(
                        "$.data.accounts[1].accountId",
                        is(secondAccountId)))
                .andExpect(jsonPath(
                        "$.data.accounts[1].accountPlatform",
                        is("BANK")))
                .andExpect(jsonPath(
                        "$.data.accounts[1].shares",
                        is(100.0)))
                .andExpect(jsonPath(
                        "$.data.accounts[1].openSellCount",
                        is(1)))
                .andExpect(jsonPath(
                ...
                .andExpect(jsonPath(
                        ...
                        is(secondAccountId)))
                .andExpect(jsonPath(
                        "$.data.openTransactions[0].status",
                        is("PENDING")))
                .andExpect(jsonPath(
    ...
    @Test
    void closedFundHasNoCurrentDetailButKeepsTransactionHistory()
            throws Exception {
        Session session = preparedPosition();
        sell(
                session.token(),
                sellBody(
                        "sell-detail-closed-001",
                        session.accountId(),
                        "FULL",
                        "1000.00",
                        "1000.00",
